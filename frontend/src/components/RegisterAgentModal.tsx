...
export default function RegisterAgentModal({ onClose, onRegistered }: RegisterAgentModalProps) {
    ...
    const [tab, setTab] = useState<"manual" | "discovery">("manual");
    ...
    const handleDiscovery = async (e: React.FormEvent) => {
        ...
        setError(null);
        try {
            const agent = await api.discoverAgent(discoveryUrl.trim());
            setSuccess(true);
            setTimeout(() => {
                onRegistered(agent);
                onClose();
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="flex p-1 bg-white/5 rounded-xl border border-white/10">
                            <button
                                ...
                                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${tab === "manual"
                                        ? "bg-white/10 text-white shadow-lg"
                                        : "text-orbit-silver/40 hover:text-orbit-silver/60"
                                    }`}
                            >
                                MANUAL
                            </button>
                            <button
                                ...
                                className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${tab === "discovery"
                                        ? "bg-white/10 text-white shadow-lg"
                                        : "text-orbit-silver/40 hover:text-orbit-silver/60"
                                    }`}
                            >
                        ...
                        {tab === "manual" ? (
                            ...
                            <form onSubmit={handleDiscovery} className="space-y-6 py-4">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] text-orbit-silver/50 uppercase tracking-widest font-bold">
                                        Agent Domain / URL <span className="text-starlight-violet">*</span>
                                    </label>
                                    <input
                                        className={FIELD_CLASSES}
                                    ...
                                    />
                                    <p className="text-[10px] text-orbit-silver/30 mt-2">
                                        AstralBridge will look for <span className="text-orbit-silver/50 font-mono">/.well-known/agent-card.json</span>
                                    </p>
                                </div>
                                ...
                                <div className="p-4 bg-starlight-violet/5 rounded-xl border border-starlight-violet/10">
                                    <p className="text-xs text-orbit-silver/60 leading-relaxed">
                                        Automated discovery follows the A2A protocol standard. The agent must provide its "business card" at the standard well-known path.
                                    </p>
                                </div>
